...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${
      scrolled ? 'bg-space-blue/95 backdrop-blur-md shadow-lg' : 'bg-space-blue/80 backdrop-blur-sm'
    }`}
    style={{backgroundImage:"/img/Group.png"}}>
      <div className="container-custom h-20 flex justify-between items-center">
        ...
        <div className="lg:hidden text-[#ff5c00] text-2xl cursor-pointer" onClick={toggleMenu}>
          {isOpen ? <FaTimes /> : <FaBars />}
        </div>

        <ul className={`fixed lg:static top-20 left-0 w-full lg:w-auto h-[calc(100vh-5rem)] lg:h-auto 
          ...
          {['Home', 'About', 'FAQ', 'Prize', 'Gallery', 'Sponsors', 'Team'].map((item) => (
